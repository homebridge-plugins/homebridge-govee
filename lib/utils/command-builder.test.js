...
describe('what goes out over AWS', () => {
  ...
  it('sends brightness as 0-100 by default, the scale most models use', () => {
    // The default was 0-254 for a while, and the list of models that had to
    // opt out grew with every release (#1364)
    expect(buildCommand({ cmd: 'brightness', value: 100 }, light).awsParams.data.val).toBe(100)
    expect(buildCommand({ cmd: 'brightness', value: 37 }, { gvModel: 'H601A' }).awsParams.data.val).toBe(37)
  ...
  it('scales up to 254 for the models that count that way', () => {
    // An owner's 100% arrived on these as 35-39% while nothing was scaled (#1262)
    expect(buildCommand({ cmd: 'brightness', value: 100 }, { gvModel: 'H6054' }).awsParams.data.val).toBe(254)
    expect(buildCommand({ cmd: 'brightness', value: 50 }, { gvModel: 'H6144' }).awsParams.data.val).toBe(127)
    expect(buildCommand({ cmd: 'brightness', value: 1 }, { gvModel: 'H6143' }).awsParams.data.val).toBe(3)
  })

  it('lets the device config override the list in either direction', () => {
    expect(buildCommand({ cmd: 'brightness', value: 100 }, { gvModel: 'H6054', awsBrightnessNoScale: true }).awsParams.data.val).toBe(100)
    expect(buildCommand({ cmd: 'brightness', value: 100 }, { gvModel: 'H6102', awsBrightnessScale: true }).awsParams.data.val).toBe(254)
    // no-scale wins if an owner somehow ticks both
    expect(buildCommand({ cmd: 'brightness', value: 100 }, { gvModel: 'H6102', awsBrightnessScale: true, awsBrightnessNoScale: true }).awsParams.data.val).toBe(100)
  })
...
describe('what goes out over the local network', () => {
  ...
  it('⚠️ sends brightness unscaled, even for a model that counts to 254 over AWS', () => {
    // The two connections genuinely differ. Making them match would send every
    // local device to full brightness
    const built = buildCommand({ cmd: 'brightness', value: 50 }, { gvModel: 'H6054' })
