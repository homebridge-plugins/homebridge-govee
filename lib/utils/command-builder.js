...
import { awsBrightnessIsScaled, getDeviceCapabilities } from './device-capabilities.js'
...
function buildBrightnessCommand(params, context) {
  ...
  return {
    awsParams: {
      cmd: 'brightness',
      // AWS brightness is 0-100 for most models, the same as HomeKit. The
      // models that count to 254 are listed in device-capabilities.js, and a
      // device can be forced either way with `awsBrightnessScale` or
      // `awsBrightnessNoScale` in its config (#1262, #1364).
      data: {
        val: awsBrightnessIsScaled(context, caps)
