...
export function applyUserConfig(platform, config) {
  ...
  const logDecrease = (k, max) => {
    platform.log.warn('%s [%s] %s %s.', platformLang.cfgItem, k, platformLang.cfgHigh, max)
  }
  const logQuotes = (k) => {
  ...
  Object.entries(config).forEach((entry) => {
    ...
    switch (key) {
      ...
      case 'openApiRefreshTime': {
        ...
        } else if (platformConsts.maxValues[key] !== undefined && intVal > platformConsts.maxValues[key]) {
          logDecrease(key, platformConsts.maxValues[key])
          platform.config[key] = platformConsts.maxValues[key]
        } else {
          ...
          val.forEach((x) => {
            ...
            entries.forEach((subEntry) => {
              ...
              switch (k) {
                ...
                case 'sceneSelector':
                case 'sceneSelectorSnapshots':
                case 'sceneSelectorDiy':
