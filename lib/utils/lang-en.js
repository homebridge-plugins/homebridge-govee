export default {
  ...
  alDisabledScene: 'adaptive lighting turned off, as a scene was selected',
  ...
  openApiTempUnitUnknown: 'ignoring the openapi temperature as it is unclear whether govee sent celsius or fahrenheit - you can set this in the plugin settings for this device',
  brand: 'Govee',
  ...
  blePowerBlocked: 'the bluetooth adapter is switched off or blocked - check `rfkill list`, and `sudo rfkill unblock bluetooth` if it is blocked',
  blePowerNoAdapter: 'no bluetooth adapter was found - check the adapter exists and the bluetooth service is running',
  blePowerUnauthorized: 'not allowed to use the bluetooth adapter - node needs raw network access, which a node upgrade wipes: `sudo setcap cap_net_raw+eip $(readlink -f $(which node))`, then fully restart homebridge',
  blePowerUnknown: 'the bluetooth adapter never reported its state - check the bluetooth service is running',
  ...
  sceneCap: (kept, total) => `the device lists ${total} scenes but HomeKit allows at most ${kept} inputs on one accessory - keeping the first ${kept}, with snapshots and diy scenes first`,
  curDirection: 'current airflow direction',
  ...
  devIsGroup: 'is a Govee app device group so will not be added to Homebridge',
  ...
  devRenamed: 'has been renamed to',
  disableClient: 'disabling client as',
  disabledInConfig: 'disabled in config',
  disabling: 'Disabling plugin',
  gwRecordStale: 'ignoring gateway record with a stale timestamp',
  gwRecordType: 'ignoring gateway record that is not a real-time status',
  ...
  zoneNeedsAws: 'this light has separately switchable zones, but the zone tiles need your Govee account username and password in the plugin settings - an API key alone is not enough for them',
  zoneNeedsApi: 'this light has separately switchable zones, but the zone tiles need a Govee API key in the plugin settings',
  ...
  noToken: 'could not obtain an account token from Govee - the account may need attention in the Govee Home app (e.g. an email verification code or accepting updated terms), or it may be served by a region this plugin does not support. If you only use Bluetooth or LAN control then this cloud login is not required',
  twoFARequired: '2FA verification required - a code has been sent to your email, please add it to the "code" field in your plugin config and restart Homebridge',
  twoFACodeInvalid: '2FA verification code was provided but login still failed - please check the code is correct and has not expired',
  noTokenExists: 'no account token has been retrieved',
  offlineBTConn: 'has been reported [disconnected] via BLE',
  offlineHTTP: 'has been reported [offline]',
  onlineHTTP: 'has been reported [online]',
  ...
  syncStillRunning: 'skipping this sync as the previous one has not finished yet',
  viaAL: 'via adaptive lighting',
  viaReport: 'via device report',
  statusCodes: 'device status codes',
  cmdAcknowledged: 'command acknowledged',
  infoBroadcast: 'informational broadcast',
}
