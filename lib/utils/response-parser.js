...
/**
 * Whether a payload is only govee's receipt for a command we sent.
 *
 * These carry a command name and a `result` and nothing else - no status at
 * all - so there is nothing in them to have failed to understand. Anything
 * that names even one other field is a real payload and is not this.
 *
 * @param {object} params - Raw payload with source, cmd, state, etc.
 * @returns {boolean} true if the payload is a bare acknowledgement
 */
export function isCommandAcknowledgement(params) {
  if (!params?.cmd || !params.state || typeof params.state !== 'object') {
    return false
  }
  const keys = Object.keys(params.state)
  return keys.length > 0 && keys.every(key => key === 'result')
}

/**
 * Payload commands that are broadcasts about the device's own goings-on, with
 * nothing in them a HomeKit tile could show.
 *
 * The camera-based TV backlights announce their video-sync mode whenever it
 * changes - a `videomode` payload saying whether sync is running and what it
 * is detecting. The plugin has no control that maps to it and there is nothing
 * the owner can do about it, so it is not worth warning anyone about - it is
 * logged at debug instead, where it can still be fetched if ever needed
 * (#1363).
 *
 * @param {object} params - Raw payload with source, cmd, state, etc.
 * @returns {boolean} true if the payload is a recognised broadcast
 */
const informationalCommands = new Set(['videomode'])

export function isInformationalBroadcast(params) {
  return typeof params?.cmd === 'string' && informationalCommands.has(params.cmd)
}
...
export function parseDeviceUpdate(params, context) {
  ...
  if (params.state && hasProperty(params.state, 'brightness')) {
    // The models that count to 254 over AWS report on that scale too, so
    // their readings come back down to HomeKit's 0-100 (#1262). Everything
    // else already reports 0-100 (#1364).
    if (params.source === 'AWS') {
      data.brightness = awsBrightnessIsScaled(context)
 ...
 *
 * The device record carries two signals. The warning message list is what the
 * sensor's own detail screen shows, and an unread "leakage alert" entry means
 * a leak nobody has dismissed yet. The record itself also carries the time of
 * the last alarm and whether it has been read - that pair is what the app's
 * home screen uses to decide whether to show a sensor as alarming, and it is
 * marked read only when the owner taps "Got it" on the alarm (#1356).
 *
 * The message list has been seen empty while a gateway-attached sensor was
 * actually sounding and its record said unread, so for those the record is
 * consulted as well. Sensors that talk to the cloud themselves keep to the
 * message list alone: the record flag was tried for them once and taken out
 * again, and there is no capture yet to say what it means on those models.
 *
 * @param {object} deviceData the record's lastDeviceData, with lastTime and read
 * @param {Array<{message: string, read: boolean}>} messages the warning messages
 * @param {object} [options]
 * @param {boolean} [options.behindGateway] whether the sensor reports via a gateway
 * @returns {boolean} true if a leak alarm has not been acknowledged
 */
export function hasUnacknowledgedLeak(deviceData, messages, { behindGateway = false } = {}) {
  const lastTime = Number(deviceData?.lastTime) || 0
  if (lastTime <= 0) {
    return false
  }

  const unreadMessage = (Array.isArray(messages) ? messages : []).some(msg => (
    !msg.read
    && typeof msg.message === 'string'
    && msg.message.toLowerCase().replace(WHITESPACE, '').startsWith('leakagealert')
  ))
  if (unreadMessage) {
    return true
  }

  return behindGateway && deviceData.read === false
}
