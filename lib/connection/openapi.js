...
import { describeControlFailure, parseError, sleep } from '../utils/functions.js'
import { isFirstReport } from '../utils/report-unknown.js'
...
export default class {
  constructor(platform) {
    ...
    // Pacing for the per-device scene list requests at startup - see getDevices
    this.sceneFetchGapMs = 250
    this.sceneRetryDelaysMs = [2000, 5000, 10000]
  }
  ...
  async request({ method, path, data }) {
    try {
      ...
      // Keep the http status, so a caller can tell a rate limit from a failure
      const wrapped = new Error(parseError(err))
  ...
  /**
   * Fetch a device's scene list, waiting out the api's rate limit if it is hit.
   *
   * The api answers a burst of requests with 429s rather than queuing them, so
   * a limit is not a failure - it is a request to slow down, and the same call
   * a few seconds later goes through. Anything other than a 429 is thrown as
   * it is.
   */
  async getScenesWithRetry(sku, device) {
    for (let attempt = 0; ; attempt++) {
      try {
        return await this.getScenes(sku, device)
      } catch (err) {
        if (err?.status !== 429 || attempt >= this.sceneRetryDelaysMs.length) {
          throw err
        }
        this.log.debug?.(`[OPENAPI] rate limited fetching scenes for ${device}, retrying in ${this.sceneRetryDelaysMs[attempt]}ms`)
        await sleep(this.sceneRetryDelaysMs[attempt])
      }
    }
  }
  ...
  async getDevices() {
    ...
    const devices = getDevicesFromPayload(data).filter(device => device?.sku && device?.device)

    // The device list names each scene capability but not its options, so the
    // scene list is a second request per device. Those used to go out all at
    // once, and past a dozen or so devices the api started answering 429 - and
    // because the failure was only logged at debug, the owner just saw scene
    // selectors silently missing for the devices at the tail of the list
    // (#1362). So: one at a time, a short gap between, a wait-and-retry on a
    // 429, and a warning that names the device if it still cannot be fetched.
    const sceneCapabilitiesByDevice = new Map()
    let fetched = 0
    for (const device of devices) {
      ...
      if (fetched > 0 && this.sceneFetchGapMs > 0) {
        await sleep(this.sceneFetchGapMs)
      }
      fetched++
      try {
        sceneCapabilitiesByDevice.set(device.device, await this.getScenesWithRetry(device.sku, device.device))
      } catch (err) {
        this.log.warn(`[OPENAPI] could not fetch the scene list for ${device.deviceName || device.device} (${err.message}), so it will have no scene selector until the next restart.`)
      }
    }
    ...
    const parsedDevices = devices.map((device) => {
      ...
      if (sceneCapabilitiesByDevice.has(device.device)) {
        capabilities = mergeSceneCapabilities(capabilities, sceneCapabilitiesByDevice.get(device.device))
      }
  ...
  getCapability(accessory, instance, fallbackType) {
    ...
    // The device never listed this one. Govee will still accept the request and
    // answer as though it worked, then do nothing - so without saying something
    // here the control simply appears broken with no clue why. Said once per
    // device and capability, then dropped to debug
    const fingerprint = `openapi-unlisted|${accessory.context.gvDeviceId}|${instance}`
    const message = `govee did not list [${instance}] for this device, so that command may be ignored - please report this line if the control does not work: ${JSON.stringify({
      model: accessory.context.gvModel,
      instance,
      listed: Object.keys(lookup),
    })}`
    if (isFirstReport(fingerprint)) {
      accessory.logDebugWarn(message)
    } else {
      accessory.logDebug(message)
    }
  ...
  async updateDevice(accessory, params) {
    ...
    // Govee can accept the request and still refuse the command, reporting it
    // only inside `capability.state`. Without this the plugin treated that as
    // success and the device quietly ignored the command (#1324).
    const refusal = describeControlFailure(response)
    if (refusal) {
      const error = new Error(refusal)
      // Their wording is aimed at the user and describes the device's state, so
      // log it on its own — a stack trace here suggests a fault in the plugin.
      error.deviceRefusal = true
      throw error
    }
  }
