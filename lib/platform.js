...
export default class {
  ...
  async sendDeviceUpdate(accessory, params) {
    ...
    switch (params.cmd) {
      ...
      case 'brightness': {
        ...
        */
        // Scale brightness from [0, 100] to [0, 254] for AWS unless awsBrightnessNoScale is true
        const awsBrightness = accessory.context.awsBrightnessNoScale
          ? params.value
          : Math.round((params.value / 100) * 254)
        data.awsParams = {
          cmd: 'brightness',
          data: {
            val: awsBrightness,
  ...
  receiveDeviceUpdate(accessory, params) {
    ...
    if (params.data && !params.state) {
      ...
      } else if (hasProperty(params.data, 'val')) {
        // Some devices may send brightness as 'val'
        params.state.brightness = params.data.val
      }
    }
    ...
    */
    if (params.state) {
      // Check for brightness property (AWS/LAN) or val property (some AWS devices)
      let brightnessValue = null
      if (hasProperty(params.state, 'brightness')) {
        brightnessValue = params.state.brightness
      } else if (hasProperty(params.state, 'val') && params.source === 'AWS') {
        // Some AWS devices may send brightness as 'val'
        brightnessValue = params.state.val
      }
      
      if (brightnessValue !== null) {
        if (params.source === 'LAN') {
          data.brightness = brightnessValue
        } else if (params.source === 'AWS') {
          // AWS sends brightness in [0, 254] range, scale to [0, 100] unless awsBrightnessNoScale is true
          let brightness = brightnessValue
          if (!accessory.context.awsBrightnessNoScale && brightness > 100) {
            // Scale from [0, 254] to [0, 100]
