...
import deviceSceneTv, { sceneSelectorOptions } from './device/scene-tv.js'
...
import { isFirstReport, logUnknownData } from './utils/report-unknown.js'
import { hasUnacknowledgedLeak, isCommandAcknowledgement, isInformationalBroadcast, parseDeviceUpdate } from './utils/response-parser.js'
...
export default class {
  ...
  logConnectionSummary() {
    ...
    // Real-time channels
    if (this.awsClient) {
      realtime.push(`AWS IoT MQTT (${awsCount} device(s))`)
    }
    if (this.openApiClient?.mqttConnected) {
      realtime.push(`OpenAPI MQTT (${openApiCount} device(s))`)
    }
    if (this.lanClient && lanCount > 0) {
      realtime.push(`LAN UDP (${lanCount} device(s))`)
    }

    // Polling intervals
    if (this.refreshAWSInterval) {
      polling.push(`AWS every 60s (${awsCount} device(s))`)
    }
  ...
  initialiseDevice(device) {
    ...
    try {
      ...
      // A device can opt into a scene selector - an extra television-style
      // accessory whose input list is the device's own scene list (#1360)
      this.initialiseSceneSelector(device, deviceConf, accessory)
    } catch (err) {
  ...
  initialiseSceneSelector(device, deviceConf, mainAccessory) {
    try {
      const options = sceneSelectorOptions(device, deviceConf, (kept, total) => {
        this.log.warn('[%s] %s.', device.deviceName, platformLang.sceneCap(kept, total))
      })
      if (!options || !mainAccessory.context.useOpenApiControl) {
        return
      }

      // Its own device id, so the selector never collides with the light
      const accessory = this.addExternalAccessory({
        ...device,
        device: `${device.device}_scenes`,
        deviceName: `${device.deviceName} Scenes`,
      ...
      // Just enough context for the openapi client to address the device -
      // the full context machinery belongs to the main accessory
      accessory.context.gvDeviceId = device.device
      accessory.context.gvModel = device.model
      accessory.context.useOpenApiControl = true
      accessory.context.openApiCapabilities = mainAccessory.context.openApiCapabilities
      accessory.context.sceneOptions = options

      accessory.control = new deviceSceneTv(this, accessory)
      devicesInHB.set(accessory.UUID, accessory)
    } catch (err) {
      this.log.warn('[%s] %s %s.', device.deviceName, platformLang.devNotInit, parseError(err))
    }
      ...
      await this.bleClient.startDiscovery((goveeReading) => {
        try {
          ...
          if (accessory && !platformConsts.models.sensorMonitor.includes(accessory.context.gvModel)) {
            ...
            // A monitor's readings arrive over its own connection, so there is
            // nothing to do with this one and nothing for the owner to act on
            accessory.logDebug(`[BLE] ${platformLang.bleScanUnknown}`)
          } else if (isFirstReport(`ble-reading|${goveeReading.address}`)) {
            // A sensor broadcasts every few seconds, so say this once per session
            // rather than on every advertisement. Only an address that belongs to
            // this account is worth a warning - anything else may be a neighbour's
            // device, which the owner cannot do anything about.
            const line = `[BLE] ${platformLang.bleScanUnknown} [${goveeReading.address}].`
            if (this.hasBleSensor(goveeReading.address)) {
              this.log.warn(line)
            } else {
              this.log.debug(line)
            }
          ...
          try {
            ...
            if (platformConsts.models.sensorLeak.includes(device1.sku)) {
              ...
              if (parsedData.lastTime > 0) {
                ...
                // An unread message means a leak; so does an unread record on a
                // sensor behind a gateway - see hasUnacknowledgedLeak (#1356)
                hasUnreadLeak = hasUnacknowledgedLeak(parsedData, msgs, {
                  behindGateway: !!accessory.context.gatewayDevice,
                })
              }
  ...
  /**
   * Whether a bluetooth address belongs to one of this account's sensors.
   *
   * An undecodable broadcast is only worth putting in front of someone when it
   * came from a sensor of theirs, because a sensor is the one kind of device
   * whose readings arrive this way and so the one kind the plugin is failing.
   * The two cases this deliberately leaves quiet:
   *
   * - a light or appliance of theirs, which also broadcasts constantly but is
   *   controlled over aws/lan/ble and has nothing to gain from being decoded
   * - an address that is not on this account at all, which may be a neighbour's
   *   device and is not something the owner can act on
   */
  hasBleSensor(address) {
    if (!address) {
      return false
    }
    const wanted = String(address).toLowerCase()
    // Any list named sensor* counts, so a new sensor type is covered on the day
    // it is added rather than needing this list updated too
    const sensorModels = Object.keys(platformConsts.models)
      .filter(key => key.startsWith('sensor'))
      .flatMap(key => platformConsts.models[key])
    return [...devicesInHB.values()].some(
      accessory => String(accessory.context.bleAddress || '').toLowerCase() === wanted
        && sensorModels.includes(accessory.context.gvModel),
    )
  }
  ...
  receiveDeviceUpdate(accessory, params) {
    ...
    if (Object.keys(data).length > 1) {
      ...
      // A scene selector companion mirrors the light's power state, so the
      // dropdown is usable whenever the light is on (#1360). Updates route by
      // the device's own id, so the companion never receives them directly
      if (!accessory.context.sceneOptions) {
        const companion = devicesInHB.get(this.api.hap.uuid.generate(`${accessory.context.gvDeviceId}_scenes`))
        if (companion?.control?.externalUpdate) {
          try {
            companion.control.externalUpdate(data)
          } catch (err) {
            this.log.warn('[%s] %s %s.', companion.displayName, platformLang.devNotUpdated, parseError(err))
          }
        }
      }
    } else if (isCommandAcknowledgement(params)) {
      // Govee answers a command with a receipt carrying nothing but whether it
      // worked. There is no status in it to miss, so reporting it as a payload
      // nobody understood buries the ones that are worth reading (#1352)
      accessory.logDebug(`[${params.source}] ${platformLang.cmdAcknowledged} [${params.cmd}]`)
    } else if (isInformationalBroadcast(params)) {
      // The device talking about itself, with nothing a tile could show - see
      // informationalCommands in the response parser (#1363)
      accessory.logDebug(`[${params.source}] ${platformLang.infoBroadcast} [${params.cmd}] ${JSON.stringify(params.state ?? {})}`)
    } else {
