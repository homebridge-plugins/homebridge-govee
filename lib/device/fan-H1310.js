...
export default class GoveeFanH1310 extends deviceFanH1370 {
  ...
  async internalDirectionUpdate(value) {
    try {
      ...
      this.accessory.log(`${platformLang.curDirection} [${value === 1 ? 'reverse' : 'forward'}]`)
    } catch (err) {
      this.failUpdate(err, () => {
  ...
  externalUpdate(params) {
    ...
    if (this.canReverseAirflow && params.toggles?.[REVERSE_AIRFLOW] !== undefined) {
      const newDirection = params.toggles[REVERSE_AIRFLOW] ? 1 : 0
      if (this.cacheDirection !== newDirection) {
        this.cacheDirection = newDirection
        this.service.updateCharacteristic(this.hapChar.RotationDirection, newDirection)
        this.accessory.log(`${platformLang.curDirection} [${newDirection === 1 ? 'reverse' : 'forward'}]`)
      }
    }
  }
