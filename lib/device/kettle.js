...
export default class extends GoveeDevice {
  ...
  externalUpdate(params) {
    ...
    (params.commands || []).forEach((command) => {
      ...
      // On startup the kettle lists the temperature each of its presets is set
      // to, one frame per preset: aa 05 <slot> <degrees F>. That is a different
      // thing from aa 05 00 <mode> below, which is a mode change - the slot
      // byte is what tells them apart. Nothing here needs the values, so they
      // are logged for reference rather than reported as unrecognised (#1337)
      if (getTwoItemPosition(hexParts, 2) === '05' && getTwoItemPosition(hexParts, 3) !== '00') {
        ...
        const line = `preset ${slot} is set to ${tempInF}°F [${Math.round(((tempInF - 32) * 5) / 9)}°C]`
