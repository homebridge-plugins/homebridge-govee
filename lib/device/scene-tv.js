...
// An accessory may hold at most 100 services (HAP throws on the 101st). The
// accessory information and television services use two of those, so 98
// inputs is the most one selector can carry. The sort below puts the user's
// own snapshots and diy scenes before the stock catalogue, so a capped
// device loses stock scenes, never personal ones (#1362)
export const MAX_SCENE_OPTIONS = 98
 ...
 * @param {(kept: number, total: number) => void} [onCap] called when the list was cut to MAX_SCENE_OPTIONS
 * @returns {Array<{ name: string, value: any, instance: string }> | null} the merged options, or null when the selector should not exist
 */
export function sceneSelectorOptions(device, deviceConf, onCap) {
  const merged = []
  ...
  if (merged.length > MAX_SCENE_OPTIONS) {
    onCap?.(MAX_SCENE_OPTIONS, merged.length)
    merged.length = MAX_SCENE_OPTIONS
  }
