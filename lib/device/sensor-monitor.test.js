...
describe('the monitor has no battery tile', () => {
  // The H5106 is USB powered with no battery at all - the tile v11.39.0 gave
  // it never received a reading, so its 0% showed as a permanent Battery Low
  // in the home app (#1296)
  it('never adds a battery service', () => {
    const device = build()

    expect(device.accessory.getService(device.hapServ.Battery)).toBeUndefined()
  })

  it('removes the battery service v11.39.0 wrongly added to existing accessories', () => {
    const accessory = makeAccessory('H5106', { offTemp: 0, offHumi: 0 })
    accessory.addService('Battery')

    const device = new deviceSensorMonitor(makePlatform(), accessory)

    expect(accessory.getService(device.hapServ.Battery)).toBeUndefined()
  })

  it('ignores a battery field arriving in the params', async () => {
    const device = build()

    await device.externalUpdate({ battery: 3 })
