...
export default class extends GoveeDevice {
  constructor(platform, accessory) {
    ...
    // Mark it as the primary service so the HomeKit tile shows the temperature
    ...
    // The H5106 runs on USB power and has no battery at all (#1296).
    // v11.39.0 wrongly gave it a battery tile, and with no reading ever
    // arriving its empty 0% surfaced in the home app as a permanent Battery
    // Low - so remove the service from any accessory that already carries
    // it, and never add one again
    const battService = this.accessory.getService(this.hapServ.Battery)
    if (battService) {
      this.accessory.removeService(battService)
    }
  ...
  async externalUpdate(params) {
    // Readings can also arrive from the HTTP device list (#1322), for accounts
    // where the AWS channel never delivers for this model. The fields use the
    // same semantics as the thermo-hygrometer sensors: hundredths of a degree
    // and hundredths of a percent
    ...
    (params.commands || []).forEach((command) => {
      ...
      switch (deviceFunction) {
        ...
        default: {
          ...
          // A frame whose leading bytes decode outside any plausible room
          // reading is not a status report at all - the `33 18` command ack
          // landed here and became 130.8°C (#1296). Skip the whole frame:
          // its other byte positions are not readings either
          if (tempInCen < -30 || tempInCen > 80) {
            break
          }
