...
export default class extends GoveeDevice {
  ...
  async internalSwingUpdate(value) {
    try {
      const newValue = value ? 'on' : 'off'
      // Don't continue if the new value is the same as before. The comparison
      // has to be against the mapped string - cacheSwing holds 'on'/'off' and
      // value arrives as a boolean, so comparing them directly never matched
      if (this.cacheSwing === newValue) {
        return
      }

      // Same oscillation shape as the H7107, which is the only other fan that
      // reports a swing RANGE rather than a plain on/off. Turning ON carries
      // over the range bytes from the last report seen; turning OFF must not,
      // because a parameterised off command was answered with a zero-tailed
      // frame while the fan kept oscillating (#1334, #1339)
      ...
      // The fan's own oscillation state travels as multiSync traffic, so send
      // that shape too. Whichever the model ignores is harmless
      await this.platform.sendDeviceUpdate(this.accessory, {
        cmd: 'multiSync',
